...
public class VeiculoFormMB extends AppFormMB<VeiculoED, Long> {
	...
	public String[] getListaAnosFabricacao() {
		return listaAnosFabricacao;
	}

	public void setListaAnosFabricacao(String[] listaAnosFabricacao) {
		this.listaAnosFabricacao = listaAnosFabricacao;
	}

	// // NÃO EXECUTA SE FOR AJAX
	// if (!FacesContext.getCurrentInstance().isPostback()) {
	// if (!isInclusao()) {
	// this.listaHistoricoJunta =
	// juntaAgHistRN.getHistoricoJunta(getEd().getId());
	// }
	//
	...
	// localAtuacaoED =
	// cronogramaRN.getLocalAtuacao(super.getEd().getMunicipioED().getCodMunicipio(),
	// "M");
	// else
	// localAtuacaoED =
	// cronogramaRN.getLocalAtuacao(super.getEd().getMunicipioED().getCodMunicipio(),
	...
	// this.listaSubstituicoes =
	// cronogramaRN.getListaSubstituicoes(super.getEd().getId());
	//
	...
	// if
	// (profs01.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent()
	// != null)
	// if
	// ((profs01.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent().equals(ed.getProfissionalAntED().getProfissionaisPK().getDocIdent())))
	// {
	// labelProfs01 = "Substituto";
	// }
	// if
	// (profs02.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent()
	// != null)
	// if
	// ((profs02.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent().equals(ed.getProfissionalAntED().getProfissionaisPK().getDocIdent())))
	// {
	// labelProfs02 = "Substituto";
	// }
	// if
	// (profs03.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent()
	// != null)
	// if
	// ((profs03.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent().equals(ed.getProfissionalAntED().getProfissionaisPK().getDocIdent())))
	// {
	...
	// JuntaComissaoED juntaComissaoED =
	// juntaComissaoRN.getComissaoAtiva(TipoComissaoEnum.MEDICA_ESPECIAL_DETRAN.getCodigo());
	// if (juntaComissaoED != null)
	// super.getEd().setJuntaComissaoED(juntaComissaoED);
	// else
	// FacesUtil.addErrorMessage("Atenção! Não existe uma comissão ativa para
	// Junta Médica Especial. Favor verificar.");
	// }
	//
	...
	// throw new RNException("Atenção! Município não permite atendimento no
	// turno da manhã.");
	// }
	//
	// // TARDE
	...
	// if (!isInclusao())
	// listaProfsCronograma =
	// agendaProfissionalRN.getListaProfissionaisVinculadosAoCronograma(super.getEd().getId());
	//
	...
	// } else {
	// FacesUtil.addErrorMessage("Atenção! Publicação deve ocorrer somente para
	// cronograma com situação 'EM ANÁLISE'");
	// }
	// }
	//
	...
	// FacesUtil.addInfoMessage("Atenção! Cancelamento do turno executado com
	// sucesso!");
	...
	//
	// /**
	// * Método que monta mensagm de confrmação de cancelamento, considerando se
	// o
	...
	// return "Existem " + getEd().getAtendAgend() + " agendamentos. Confirma
	// cancelamento?";
	// } else {
	...
	// if
	// (cronogramaRN.verifidaDiaUtileFeriado(HabWebUtil.samdToString(ed.getSamdAgenda())))
	// {
	// /*
	...
	// // DA JUNTA
	// if (cronogramaRN.validaDataEncerramentoComissao(ed.getJuntaComissaoED(),
	// ed.getSamdAgenda())) {
	// FacesUtil.addInfoMessage("A data de agendamento informada é maior que a
	// data de vencimento da Junta. Favor verificar.");
	// return;
	// }
	//
	...
	//
	// // TESTA PROFS01
	// municipio = this.validaPeritoAtuandoEmOutraJunta(profs01,
	// ed.getSamdAgenda());
	// if (municipio != null) {
	//
	...
	//
	// // TESTA PROFS02
	// municipio = this.validaPeritoAtuandoEmOutraJunta(profs02,
	// ed.getSamdAgenda());
	// if (municipio != null) {
	//
	// // SOMENTE SE MUNICIPIO DIFERENTE
	...
	//
	// // TESTA PROFS03
	// municipio = this.validaPeritoAtuandoEmOutraJunta(profs03,
	// ed.getSamdAgenda());
	// if (municipio != null) {
	//
	// // SOMENTE SE MUNICIPIO DIFERENTE
	...
	// }
	//
	// private void showMessageValidacaoJunta(AtividadeProfissionalED prof,
	// String municipio) {
	//
	// mensagemValidaJunta = "O perito <" + prof.getNome() + ">, RG<" +
	// prof.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent()
	// + ">" + " está escalado para atuar em menos de 72 horas na cidade <" +
	// municipio + ">. Deseja continuar?";
	//
	// RequestContext context = RequestContext.getCurrentInstance();
	...
	// }
	//
	// private JuntaCronogramaProfsED
	// getAgendaProfissionaisCronograma(AtividadeProfissionalED prof) {
	...
	// }
	//
	// private String validaPeritoAtuandoEmOutraJunta(AtividadeProfissionalED
	// profs, Integer samd) {
	// return
	// agendaProfissionalRN.validaProfissionalEmOutraJunta(profs.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent(),
	// profs.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getUfIdent(),
	// Util.paraCalendar(samd - 3));
	// }
	//
	// // VERIFICA SE OS 3 PROFISSIONAIS FORAM INFORMADOS E SE SAO DIFERENTES
	...
	// FacesUtil.addInfoMessage("Atenção! Os 3 profissionais informados devem
	// ser diferentes.");
	// return true;
	// }
	//
	...
	// listaProfsCronograma =
	// cronogramaRN.sugestaoProfsNovoCronograma(codMunicipio);
	// carregouProfs = true;
	// this.setProfissionaisCronograma();
	// }
	//
	...
	// FacesUtil.addInfoMessage("Atenção! O cronograma será regerado nas
	// próximas 24 horas.");
	// }
	//
	...
	// mensagemTransferenciaCronograma = "Já existe cronograma para a mesma
	// data, turno e município escolhido. Confirma a alteração de data para o
	// dia " + super.getEd().getDthAgenda() + "? Lembrete: O aviso aos
	// condutores agendados, profissionais e CFC deverá ser feito manualmente
	// pelo DETRAN.";
	// } else {
	//
	// // VERIFICA OS 3 PROFISSIONAIS
	// ProfissionalED profED01 =
	// listaProfsCronograma.get(0).getProfissionalED();
	// ProfissionalED profED02 =
	// listaProfsCronograma.get(1).getProfissionalED();
	// ProfissionalED profED03 =
	// listaProfsCronograma.get(2).getProfissionalED();
	//
	// String municipio01 =
	// agendaProfissionalRN.validaProfissionalEmOutraJunta(profED01.getProfissionaisPK().getDocIdent(),
	// profED01.getProfissionaisPK().getUfIdent(),
	// super.getEd().getDthAgenda());
	// String municipio02 =
	// agendaProfissionalRN.validaProfissionalEmOutraJunta(profED02.getProfissionaisPK().getDocIdent(),
	// profED02.getProfissionaisPK().getUfIdent(),
	// super.getEd().getDthAgenda());
	// String municipio03 =
	// agendaProfissionalRN.validaProfissionalEmOutraJunta(profED03.getProfissionaisPK().getDocIdent(),
	// profED03.getProfissionaisPK().getUfIdent(),
	// super.getEd().getDthAgenda());
	//
	// if (municipio01 != null) {
	// mensagemTransferenciaCronograma = "O profissional '" + profED01.getNome()
	// + "' encontra-se na junta de '" + municipio01 + "' no mesmo dia
	// escolhido. Confirma a alteração de data para o dia " +
	// super.getEd().getDthAgenda() + "? Lembrete: O aviso aos condutores
	// agendados, profissionais e CFC deverá ser feito manualmente pelo
	// DETRAN.";
	// } else if (municipio02 != null) {
	// mensagemTransferenciaCronograma = "O profissional '" + profED02.getNome()
	// + "' encontra-se na junta de '" + municipio02 + "' no mesmo dia
	// escolhido. Confirma a alteração de data para o dia " +
	// super.getEd().getDthAgenda() + "? Lembrete: O aviso aos condutores
	// agendados, profissionais e CFC deverá ser feito manualmente pelo
	// DETRAN.";
	// } else if (municipio03 != null) {
	// mensagemTransferenciaCronograma = "O profissional '" + profED03.getNome()
	// + "' encontra-se na junta de '" + municipio03 + "' no mesmo dia
	// escolhido. Confirma a alteração de data para o dia " +
	// super.getEd().getDthAgenda() + "? Lembrete: O aviso aos condutores
	// agendados, profissionais e CFC deverá ser feito manualmente pelo
	// DETRAN.";
	// } else {
	// mensagemTransferenciaCronograma = "Confirma a alteração de data para o
	// dia " + super.getEd().getSamdAgendaFormatada() + "? Lembrete: O aviso aos
	// condutores agendados, profissionais e CFC deverá ser feito manualmente
	// pelo DETRAN.";
	// }
	// }
	...
	// FacesUtil.addInfoMessage("Atenção! A data de agendamento foi alterada com
	// sucesso.");
	// }
	//
	// public boolean validaCredenciadoInformado() {
	// if (super.getEd().getCredenciadoED().getCodCliente() != null) {
	// ComissaoLocalAtuacaoED ed =
	// comissaoLocalAtuacaoRN.getLocalAtuacaoPorCredenciado(super.getEd().getCredenciadoED().getCodCliente());
	//
	// if (ed == null) {
	//
	// RequestContext context = RequestContext.getCurrentInstance();
	...
	// FacesUtil.addErrorMessage("Atenção! Selecionar ao menos um registro para
	// imprimir o Questionário de Anamnese");
	// return null;
	// }
	//
	...
	//
	// JuntaAgendamentoCondutorED agCondED =
	// listaAgendamentosSelecionados.get(i);
	// JuntaCronogramaED cronogramaED =
	// listaAgendamentosSelecionados.get(i).getJuntaCronogramaED();
	// IndividuoED individuoED =
	// listaAgendamentosSelecionados.get(i).getIndividuoED();
	//
	// report = new ReportED("/reports/QuestionarioAnammese.jasper", lista);
	// report.addParameter("P_NOME_CANDIDATO", individuoED.getNome());
	// report.addParameter("P_SEXO",
	// HabWebUtil.converteSexo(individuoED.getSexo()));
	// report.addParameter("P_IDADE",
	// HabWebUtil.calculaIdade(individuoED.getSamdNasc()).toString() + " anos");
	// report.addParameter("P_IDENTIDADE", individuoED.getIdentidade());
	// report.addParameter("P_UF", individuoED.getUfIdent());
	// report.addParameter("P_CATEGORIA_ATUAL",
	// individuoED.getCategoriaAtual());
	//
	// // Dados do agendamento
	// report.addParameter("P_LOCAL_AGEND",
	// cronogramaED.getMunicipioED().getNomeMunicipio() + " - " +
	// cronogramaED.getCredenciadoED().getRazaoSocial());
	// report.addParameter("P_DATA_AGEND", cronogramaED.getSamdAgendaFormatada()
	// + " - " + agCondED.getHmAgendaFormatada());
	// report.addParameter("P_TURNO_AGEND", cronogramaED.getTurno() == 1 ?
	// "Manhã" : "Tarde");
	//
	// // Busca Renach ativo
	// RenachED renachED = renachRN.getRenachAtivo(individuoED.getCodIndiv());
	//
	// report.addParameter("P_CATEG_PRETEN",
	// this.getCategoriaPretendida(renachED));
	//
	// // SETA OS SERVIÇOS
	...
	// context.getExternalContext().getResourceAsStream("/WEB-INF/classes/reports/QuestionarioAnammese.jasper");
	//
	// JRPrintPage paginasGerada = null;
	// try {
	// paginasGerada = (JRPrintPage) JasperFillManager.fillReport(reportStream,
	// report.getParameters(), new JREmptyDataSource()).getPages().get(0);
	// } catch (JRException e) {
	...
	//
	// ReportED report = new
	// ReportED("/reports/RelAgendamentosCronograma.jasper", lista);
	// ComissaoLocalAtuacaoED localAtuacaoED =
	// comissaoLocalAtuacaoRN.getLocalAtuacao(super.getEd().getMunicipioED().getCodMunicipio(),
	// super.getEd().getTurno());
	//
	// byte[] pdf = null;
	// JasperPrint jasperPrint = null;
	// FacesContext context = FacesContext.getCurrentInstance();
	...
	// String titulo = "AGENDAMENTOS: " +
	// super.getEd().getMunicipioED().getNomeMunicipio() + " - " +
	// localAtuacaoED.getCredenciadoED().getRazaoSocial() + " - " +
	// super.getEd().getSamdAgendaFormatada() + " - " +
	// super.getEd().getHmInicioFormatado();
	//
	// report.addParameter("P_TITULO", titulo);
	// report.addParameter("P_RODAPE",
	// dadosSessaoUtil.montaDadosSessao().getSOE_SiglaOrganizacao() + " - " +
	// dadosSessaoUtil.montaDadosSessao().getSOE_Matricula());
	// report.addParameter("P_TOTAL_AGENDAMENTOS", totalAgendamentos);
	// report.addParameter("P_PERITO1",
	// listaProfsCronograma.get(0).getProfissionalED().getNome());
	// report.addParameter("P_PERITO2",
	// listaProfsCronograma.get(1).getProfissionalED().getNome());
	// report.addParameter("P_PERITO3",
	// listaProfsCronograma.get(2).getProfissionalED().getNome());
	//
	// StringBuilder textoObs = new StringBuilder();
	// Boolean obs1 = false, obs2 = false, obs3 = false;
	//
	// List<JuntaAgendamentoCondutorED> listaAgendamentosRel = new
	// ArrayList<JuntaAgendamentoCondutorED>();
	...
	// ed.setUltimoRenach(ed.getUltimoRenach() + " " +
	// this.getCombServico(individuoED));
	//
	// GID958RetornoED retorno958ED =
	// juntaAgendamentoCondutorRN.validadeCNH(ed);
	//
	...
	// Integer varDataValidadeCNH =
	// HabWebUtil.getSamdMaisSomaDeDias(individuoED.getSamdValid(), 30);
	...
	//
	// if (obs1)
	// textoObs.append("(1) Condutor com documento válido e passível de
	// recolhimento na data do exame.\n");
	// if (obs2)
	// textoObs.append("(2) Condutor com documento já recolhido.\n");
	// if (obs3)
	// textoObs.append("(3) Documento com recolhimento pendente.\n");
	//
	// textoObs.append("Índices de serviços: (1º) Primeira Habilitação, (R)
	// Renovação, (D) CNH Definitiva, (M) Mudança de categoria, (A) Adição de
	// Categoria, (O) Outros Serviços.");
	...
	// JRBeanCollectionDataSource(listaAgendamentosRel);
	//
	// InputStream reportStream =
	// context.getExternalContext().getResourceAsStream("/WEB-INF/classes/reports/RelAgendamentosCronograma.jasper");
	// try {
	// jasperPrint = JasperFillManager.fillReport(reportStream,
	// report.getParameters(), ds);
	// pdf = JasperExportManager.exportReportToPdf(jasperPrint);
	...
	// InputStream is = new ByteArrayInputStream(pdf);
	// return new DefaultStreamedContent(is, "application/pdf",
	// "RelatorioAgendamentosCronograma.pdf");
	// }
	//
	...
	// // RELATORIO SERÁ ABERTO NO VB ENTÃO PRECISAMOS FAZER O DONWLOAD DO
	// ARQUIVO
	// /* NO XHTML USAMOS:
	// * <h:commandLink
	// value="Teste"
	...
	// }
	//
	// FacesContext facesContext = FacesContext.getCurrentInstance(); //Get the
	// context ONCE
	...
	//
	// response.addHeader("Content-disposition", "inline;
	// filename=\"QuestionarioAnammese.pdf\"");
	// byte[] bytes = pdf;
	//
	...
	// listaAgendamentos =
	// juntaAgendamentoCondutorRN.getCondutoresAgendadosNoTurno(super.getId());
	//
	...
	// }
	//
	// public List<JuntaAgendamentoCondutorED>
	// getListaAgendamentosSelecionados() {
	// return listaAgendamentosSelecionados;
	// }
	//
	// public void
	// setListaAgendamentosSelecionados(List<JuntaAgendamentoCondutorED>
	// listaAgendamentosSelecionados) {
