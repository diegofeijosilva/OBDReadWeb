...
public class UsuarioBD extends AppBD<UsuarioED, Long> {

	public UsuarioED findByEmailSenha(String email, String senha) {
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT * FROM TBL_USUARIO A WHERE A.EMAIL = :email AND A.SENHA = :senha");
	...
	public UsuarioED findByEmail(UsuarioED ed) {
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT * FROM TBL_USUARIO A WHERE A.EMAIL = :userEmail");

		Query query = super.getEntityManager().createNativeQuery(sql.toString(), UsuarioED.class);
		query.setParameter("userEmail", ed.getEmail());
		try {
			return (UsuarioED) query.getSingleResult();
		} catch (NoResultException e) {
			return null;
	...
	// if(ped.getItensSituacaoSelecionados()!=null &&
	// ped.getItensSituacaoSelecionados().length>0){
	...
	// dc.add(Restrictions.sqlRestriction("COD_MUNIC = " +
	// ped.getMunicipioED().getCodMunicipio()));
	//
	// // FILTRO CREDENCIADO
	// if (ped.getCredenciadoED() != null){
	// if (ped.getCredenciadoED().getCodCliente() != null){
	// dc.add(Restrictions.sqlRestriction("COD_EMPRESA = '" +
	// ped.getCredenciadoED().getCodCliente().toUpperCase()+"'"));
	// }
	// }
	//
	//
	...
	// sql.append(" WHERE K206_TP_PROFIS = 29 AND K206_STATUS = 2 AND K206_NOME
	// LIKE '%" + nome + "%'");
	// sql.append(" ORDER BY K206_NOME ");
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// AtividadeProfissionalED.class);
	//
	// return (List<AtividadeProfissionalED>) query.getResultList();
	// }
	//
	// // RETORNA UM PROFISSIONAL FILTRADO POR IDENTIDADE E TP PROFISSAO
	// public AtividadeProfissionalED getProfissionalPorDocumento(Integer
	// tpProfis, String docIdentProfis, String ufDocIdentProfis) {
	// StringBuffer sql = new StringBuffer();
	// sql.append(" SELECT * ");
	// sql.append(" FROM KGID206D");
	...
	// }
	//
	// public String buscaEmailDoProfissional(String docIdent, String
	// ufDocIdent) {
	// String email = "";
	// StringBuffer sql = new StringBuffer();
	...
	// }
	//
	// public List<Long> buscaCodIndivDosCandidatosAgendados(Long
	// nroIntJuntaCronograma) {
	...
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// JuntaAgendamentoCondutorED.class);
	...
	// }
	//
	// public List<String> buscaListaDeCodClienteFromConjuntoDeCodIndiv(String
	// listCodIndiv) {
	...
	// Query query =
	// super.getEntityManager().createNativeQuery(sqlSB.toString(),
	// CredenciadoED.class);
	...
	// try {
	// String sql = "SELECT * FROM HAB_JUNTA_CRONOGRAMA B WHERE B.COD_MUNIC =
	// :codMunic AND B.DTH_AGENDA = :dthAgenda AND B.TURNO = :turno AND
	// B.SITUACAO NOT IN(9)";
	// Query query = super.getEntityManager().createNativeQuery(sql,
	// JuntaCronogramaED.class);
	...
	// }
	//
	// // Busca todos os registros na HAB_JUNTA_CRONOGRAMA FILTRANDO PELO
	// INTERVALO DE DATAS
	// public List<JuntaCronogramaED> getCronogramaPublicado(Calendar
	// dataInicio, Calendar dataFim) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA B WHERE B.DTH_AGENDA >=
	// :dataInicio");
	...
	// }
	//
	// public CronogramaParametrosED atualizaParametros(CronogramaParametrosED
	// ed) {
	...
	// }
	//
	// public CronogramaParametrosED getCronogramaParametros(String
	// abrevPropriedade) {
	// StringBuffer sql = new StringBuffer();
	...
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// CronogramaParametrosED.class);
	// query.setParameter("abrevPropriedade", abrevPropriedade);
	//
	// return (CronogramaParametrosED) query.getSingleResult();
	// }
	//
	// public List<JuntaCronogramaED> verificaTurnoMesmoDia(JuntaCronogramaED
	// ed) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA B WHERE B.SITUACAO IN
	// (1,7) ");
	// sql.append(" AND B.COD_MUNIC = :codMunic AND B.TURNO = :turno AND
	// B.DTH_AGENDA = :dthAgenda ");
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// JuntaCronogramaED.class);
	// query.setParameter("codMunic", ed.getMunicipioED().getCodMunicipio());
	...
	// }
	//
	// public List<JuntaCronogramaED> getListaTotalDeTurnosNoMunicipio(Integer
	// samdInicial, Integer samdFinal, Integer codMunicipio) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA B WHERE B.SITUACAO NOT IN
	// (9) ");
	// sql.append(" AND B.DTH_AGENDA >= :dthInicial AND B.DTH_AGENDA <=
	// :dthFinal ");
	...
	// }
	//
	// public List<JuntaCronogramaED> getListaTurnosDisponiveisMunicipio(Integer
	// samdInicial, Integer samdFinal, Integer codMunic) {
	// StringBuffer sql = new StringBuffer();
	//
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA B WHERE B.SITUACAO IN
	// (1,7) ");
	...
	//
	// /**
	// * Retorna os turnos que deverão ser encerrados no dia atual. Considera na
	// busca o dia atual. Ex.: DTH_AGENDA <= :dthAgenda
	// * @param samdAtual
	// * @return
	// */
	// public List<JuntaCronogramaED> getListaCronogramaASerEncerrado(Integer
	// samdAtual) {
	// String sql = "SELECT * FROM HAB_JUNTA_CRONOGRAMA B WHERE B.DTH_AGENDA <=
	// :dthAgenda AND B.SITUACAO IN (1,2,7)";
	// Query query = super.getEntityManager().createNativeQuery(sql,
	// JuntaCronogramaED.class);
	...
	// }
	//
	// // Retorna todos os profissionais da comissão ativa ordenados pelo
	// doc-ident
	// @SuppressWarnings("unchecked")
	// public List<AtividadeProfissionalED> getProfissonaisComissaoAtiva(String
	// profsComissaoAtiva) {
	...
	// doc-ident
	// @SuppressWarnings("unchecked")
	// public List<AtividadeProfissionalED>
	// getProfissonaisQuePodemAtuarNoMes(Integer anoAtual, Integer mesAtual,
	// String profsComissaoAtiva) {
	//
	...
	// StringBuilder sql = new StringBuilder();
	// sql.append(" SELECT * FROM KGID206D A ");
	// sql.append(" WHERE A.K206_TP_PROFIS = 29 AND A.K206_STATUS = 2 AND
	// A.K206_SAMD_VEN > :dtInicial");
	// sql.append(" AND A.K206_DOC_IDENT IN (" + profsComissaoAtiva + ") ");
	// sql.append(" ORDER BY TO_NUMBER(A.K206_DOC_IDENT) ");
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	...
	// }
	//
	// public List<CronogramaDistribuicaoMinimaProfisED>
	// getProfQueRecebemUmTurnoAMais() {
	// StringBuilder sql = new StringBuilder();
	// sql.append(" SELECT * ");
	...
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// CronogramaDistribuicaoMinimaProfisED.class);
	//
	// return query.getResultList();
	// }
	//
	// // RETORNA OS PROFISSIONAIS DISTRIBUIDOS NO MES ANTERIOR - SÓ SERVE PARA
	// OS TURNOS DO INTERIOR.
	// // PORTO ALEGRE NÃO É INSERIDO NESSA TABELA
	// public List<CronogramaDistribuidoMunicMesED>
	// getProfSDistribuidosMesAnterior(Integer anoAtual, Integer mesAtual,
	...
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// CronogramaDistribuidoMunicMesED.class);
	...
	// }
	//
	// public Integer obtemHoraInicialViaHabLocalAtuacao(Integer codMunic,
	// Integer turno) {
	//
	// Query query;
	...
	// throw new RuntimeException("Mais de um registro LocalAtuacao para o
	// municipio:" + codMunic + " de turno:" + turno);
	// }
	//
	// }
	//
	// public List<JuntaCronogramaED> getTurnosSituacao(Integer situacao) {
	// String hql = "SELECT j FROM JuntaCronogramaED j WHERE j.situacao =
	// :situacao ORDER BY j.municipioED";
	...
	// }
	//
	// // Verifica no período se o profissíonal se encontra afastado no dia a
	// ser
	// // agendado
	// public Boolean verificaProfissionalAfastado(Calendar diaAgendamento,
	// AtividadeProfissionalED atividadeProfissionalED) {
	//
	// Boolean ret = false;
	// StringBuilder sql = new StringBuilder();
	//
	// sql.append("SELECT AFAST.Nro_Int_Afast_Profis_Junta FROM
	// HAB_AFASTAMENTO_PROFIS_JUNTA AFAST ");
	// sql.append(" WHERE AFAST.DOC_IDENT_PROFIS = :docIdent AND
	// AFAST.UF_DOC_IDENT_PROFIS = :ufDocIdent ");
	// sql.append(" AND AFAST.SITUACAO = 0 ");
	// sql.append(" AND (:diaAgendamento >= AFAST.DTH_INICIO AND :diaAgendamento
	// <= AFAST.DTH_FIM) ");
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString());
	// query.setParameter("diaAgendamento", diaAgendamento);
	// query.setParameter("docIdent",
	// atividadeProfissionalED.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getDocIdent());
	// query.setParameter("ufDocIdent",
	// atividadeProfissionalED.getAtividadeProfissionalPK().getProfissionalED().getProfissionaisPK().getUfIdent());
	//
	// List<Object[]> listaArrayObj = query.getResultList();
	//
	...
	// if (listaArrayObj.size() > 0) {
	// logger.warn("*** PROFISSIONAL: " + atividadeProfissionalED.getNome() + "
	...
	// public List<JuntaCronogramaED> getListaCronogramaASerEncerrado(Calendar
	// dthInicio, Calendar dthFim) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA A WHERE (A.DTH_AGENDA >=
	// :dthInicio AND A.DTH_AGENDA <= :dthFim)");
	...
	// StringBuilder sql = new StringBuilder();
	//
	// sql.append("SELECT B.K205_EMAIL, B.K205_DOC_IDENT from KGID206D A,
	// KGID205D B");
	// sql.append(" WHERE A.K206_TP_PROFIS = 29 AND A.K206_STATUS = 2");
	...
	// }
	//
	// public List<JuntaCronogramaED>
	// buscaListaCronogramasComTurnosLotadosHoje(Calendar dthHoje) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA A WHERE A.DTH_LOTACAO =
	// :dthHoje");
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// JuntaCronogramaED.class);
	...
	// }
	//
	// public List<JuntaCronogramaED> getTurnosEmAnaliseNoPeriodo(Calendar
	// dthInicio, Calendar dthFim) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA A WHERE A.SITUACAO IN(7)
	// AND A.DTH_AGENDA > :dthInicio AND A.DTH_AGENDA < :dthFim ");
	//
	// //ATENÇÃO! NÃO REMOVER O ORDER BY. É UTILIZADO NA REDISTRIBUIÇÃO DOS
	// PROFISSIONAIS
	...
	// }
	//
	// public List<JuntaCronogramaED> getCronograma(Calendar dthInicio, Calendar
	// dthFim) {
	// StringBuffer sql = new StringBuffer();
	// sql.append("SELECT * FROM HAB_JUNTA_CRONOGRAMA A WHERE A.SITUACAO NOT
	// IN(9,8,2) AND A.DTH_AGENDA >= :dthInicio AND A.DTH_AGENDA <= :dthFim");
	//
	// Query query = super.getEntityManager().createNativeQuery(sql.toString(),
	// JuntaCronogramaED.class);
	// query.setParameter("dthInicio", dthInicio);
	...
	// String sql = "SELECT COUNT(*) AS TOTAL FROM HAB_JUNTA_AGENDAMENTO_COND A
	// WHERE A.NRO_INT_JUNTA_CRONOGRAMA = :nroIntJuntaCronograma";
	// Query query = super.getEntityManager().createNativeQuery(sql);
	// query.setParameter("nroIntJuntaCronograma", nroIntJuntaCronograma);
	//
	...
	// }
	//
	// public Boolean validaProfTurnoSituacao(String docIdent, String
	// ufDocIdent, SituacaoCronogramaEnum situacaoEnum) {
	// Integer quantidade = null;
	//
	// try {
	// StringBuffer sql = new StringBuffer();
	//
	// sql.append("SELECT COUNT(*) FROM HAB_JUNTA_CRONOGRAMA TURNO,
	// HAB_AGENDA_PROFIS PROF ");
	// sql.append(" WHERE TURNO.NRO_INT_JUNTA_CRONOGRAMA =
	// PROF.NRO_INT_AGENDA_JUNTA ");
